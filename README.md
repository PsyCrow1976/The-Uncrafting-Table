...
1. **Craft** an Uncrafting Table from 9 oak planks (3x3 grid), or find it in the Functional Blocks creative tab.
2. **Place** the block and right-click to open the GUI.
3. **Insert** a single item into the input slot on the left.
4. **Preview** the crafting recipe in the 3x3 grid on the right (with a decorative arrow between them).
5. **Cycle** through alternate recipes with the `>` button if more than one exists.
6. **Click** any preview slot to uncraft — the input is consumed and all ingredients go into your inventory.

The GUI currently uses vanilla crafting table textures. Custom art is planned for a later version.
...
2. Download `uncraftingtable-0.0.0.1.jar` from [Releases](https://github.com/PsyCrow1976/The-Uncrafting-Table/releases) or build it yourself (below).
3. Copy the JAR into your instance `mods/` folder.
4. Launch the game.
...
```
build/libs/uncraftingtable-0.0.0.1.jar
```
...
src/main/resources/
├── assets/uncraftingtable/   Models, lang, blockstates
